...
defineModel <- function() {
  model = c('ECFTX', 'NPALM', 'LWCSIM')
  res = c(1250, 704, 1000)
  xmin = c(24352.000, 680961.000, 218436.000)
  ymin = c(983097.000, 840454.000, 441788.000)
  nrows = c(603, 292, 553)
  ncols = c(740, 408, 512)
  startYr = c(1996, 1965, 1999)
  model.Params <-
    data.frame(model, res, xmin, ymin, nrows, ncols, startYr)
  rownames(model.Params) <- model.Params$model
  return(model.Params)
}
...
model.Params <-defineModel()

model='ECFTX'

res=model.Params[model,]$res
xmin =model.Params[model,]$xmin
ymin =model.Params[model,]$ymin
nrows = model.Params[model,]$nrows  
ncols = model.Params[model,]$ncols
startYr = model.Params[model,]$startYr
...
#    rchMon_1 <- t(rchMonthly[,,1,StressPeriod])*as.numeric(12*DaysPerMonth[imonth])
  ...
  if (printingOn){
    ...
    rchFile=paste("//ad.sfwmd.gov/dfsroot/data/wsd/MOD/",model,"/NetRecharge/plots/netRCH",year,"_",imonth,".png",sep="")
...
rasNames <- sprintf("RCH (inches Per Month) %s",format(seq(as.Date(paste0(startYr,"/1/1")), by = "month", length.out = SPknt),format = "%b %Y"))
...
if (Animform=='GIF'){
  ...
  ramp <-newcol(100)
  RCHgif <- paste0("//ad.sfwmd.gov/dfsroot/data/wsd/MOD/",model,"/NetRecharge/plots/netRCH.gif")
  ...
  ramp <-newcol(100)
  RCHavi <- paste0("//ad.sfwmd.gov/dfsroot/data/wsd/MOD/",model,"/NetRecharge/netRCH.avi")
